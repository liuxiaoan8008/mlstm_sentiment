...
# preprocess the data
dataset_list, num_steps = read_and_split_data(data_dir, batch_size, seq_length, num_gpus)
    ...
    with tf.Session(config=config) as sess:
        ...
        for step in xrange(num_steps):
            ...
            print("Global step: {}, progress on shard: ({}/{}), average_loss = {:.3f}, average_bpc = {:.3f}, time/batch = {:.3f}, learning_rate = {}"
                .format(gs, step,num_steps, result[1], result[1]/np.log(2) ,duration, lr))
        ...
        checkpoint_path = os.path.join(args.saved_models_dir, 'model')
        np.save(checkpoint_path, weights_list)
        print('Model weights saved')

        # save the global step at the end of the shard
        gs = sess.run(global_step)
        np.save(args.saved_models_dir + '/' + 'global_step.npy', gs)
        print('Global step saved')

        # save the losses during training
        save_logs = os.path.join(args.log_dir)
